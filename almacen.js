...
/**
 * Guarda la dirección de la foto de Open Food Facts en un producto ya guardado.
 *
 * Se usa cuando la imagen no se puede descargar: la Despensa la enseña desde
 * esa dirección mientras haya red.
 */
export async function guardarDireccion(idProducto, url) {
  const r = almacen();
  const producto = await r.obtenerProducto(idProducto);
  if (!producto) return false;
  await r.guardarProducto({
    ...producto,
    entrada: { ...(producto.entrada ?? {}), fotoUrl: url },
  });
  return true;
}
