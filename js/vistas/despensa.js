...
export function despensa() {
  ...
  const alimentos = total === 0
    ...
    : `
      ...
      ${sinFoto(cache).length > 0 ? `
        <button class="boton" id="btnBuscarFotos" style="width:100%; margin-bottom:var(--e2)">
          Buscar las fotos que faltan · ${sinFoto(cache).length}
        </button>` : ''}
      ${mensajeFotos ? `<p class="apunte-via" role="status">${esc(mensajeFotos)}</p>` : ''}
...
/**
 * Lo que dejó dicho la última búsqueda de fotos.
 *
 * Vive fuera del dibujo de la pantalla: al terminar se repinta la lista, y un
 * mensaje escrito dentro se habría borrado justo al acabar de escribirlo.
 */
let mensajeFotos = '';
...
export async function despensaActivo(raiz, { repintar }) {
  ...
  raiz.querySelector('#btnBuscarFotos')?.addEventListener('click', async (e) => {
    const boton = e.currentTarget;
    const faltan = sinFoto(cache);
    boton.disabled = true;
    let puestas = 0;
    let sinSuerte = 0;
    for (const [i, p] of faltan.entries()) {
      boton.textContent = `Buscando ${i + 1} de ${faltan.length}…`;
      const cod = p.codigoBarras ?? p.entrada?.codigoBarras;
      try {
        const r = await buscarPorCodigo(cod);
        const url = r.ok ? r.producto.imagenUrl : null;
        if (!url) { sinSuerte += 1; continue; }
        const bytes = await descargarFotoProducto(url);
        if (bytes) { await anadirFoto(p.id, bytes, 'frontal'); puestas += 1; continue; }
        // Sin poder bajarla, se guarda su dirección: se verá igual con red.
        // Sin poder bajarla se guarda su dirección en la ficha: con red se ve
        // igual, y la próxima vez ya no aparece como pendiente.
        await guardarDireccion(p.id, url);
        puestas += 1;
      } catch { sinSuerte += 1; }
    }
    mensajeFotos = puestas > 0
      ? `Puestas ${puestas} foto(s).${sinSuerte ? ` ${sinSuerte} sin foto en el catálogo.` : ''}`
      : 'Ninguno de esos productos tiene foto en Open Food Facts. Puedes hacérsela tú tocando el hueco de la foto.';
    cache = await listar({ orden: 'fecha_desc' });
...
/**
 * Los productos que se pueden completar: sin foto guardada y con código.
 *
 * Sin código no hay a quién preguntar: esos se resuelven con el botón de la
 * propia tarjeta, haciéndoles una foto.
 */
function sinFoto(lista) {
  return (lista ?? []).filter((p) =>
    !(p.fotos ?? []).some((f) => f.tipo === 'frontal')
    && !fotoDeFuera(p)
    && typeof (p.codigoBarras ?? p.entrada?.codigoBarras) === 'string');
}
